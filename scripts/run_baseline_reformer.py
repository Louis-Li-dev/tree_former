"""
scripts/run_baseline_reformer.py
=====================================
Train All-Grid Reformer (("base", "large")).
Also runs zero-shot and retrain adaptation experiments for the Large variant.

Usage
-----
    python scripts/run_baseline_reformer.py
    python scripts/run_baseline_reformer.py --sizes base
    python scripts/run_baseline_reformer.py --no-adapt
    python scripts/run_baseline_reformer.py --force
"""
...
def build_model(sz):
    s = MODEL_SIZES[sz]
    return AllGridReformer(lookback=LOOKBACK, d_model=s["d_model"],
                           nhead=s["nhead"], num_layers=s["num_layers"], d_ff=s["d_ff"])
...
def run(sizes=SIZES, force=False, do_adapt=True):
    ...
    for sz in sizes:
        ...
        save_result(run_dir, key, preds, trues, {
            "model_name": f"All-Grid Reformer ({sz.capitalize()})",
        ...
        if sz == "large" and do_adapt:
            zs_key  = "adapt_zs_reformer"
            ret_key = "adapt_retrain_reformer"
            ...
            if not result_exists(run_dir, zs_key) or force:
                ...
                save_result(run_dir, zs_key, zs_p, zs_t,
                            {"model_name": "All-Grid Reformer (Large) Zero-Shot", "MAE": zm})
            ...
            if not result_exists(run_dir, ret_key) or force:
                ...
                _, rp, rt, rpe, rte, rt_s, _ = run_training(
                    "All-Grid Reformer (Large) Retrain",
                ...
                save_result(run_dir, ret_key, rpe, rte,
                            {"model_name": "All-Grid Reformer (Large) Retrain",
