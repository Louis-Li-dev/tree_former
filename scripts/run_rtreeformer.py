...
Usage
-----
    python scripts/run_rtreeformer.py
    python scripts/run_rtreeformer.py --sizes base
    python scripts/run_rtreeformer.py --no-finetune  # skip adaptation
    python scripts/run_rtreeformer.py --force
"""
...
def finetune_rtreeformer(model, ad_train_l, ad_val_l, bc_tensor, device, logger):
...
def run(sizes=SIZES, force=False, do_finetune=True):
    ...
    for sz in sizes:
        key    = f"{SCRIPT}_{sz}"
        zs_key = "adapt_zs_rtreeformer_large"
        ft_key = "adapt_ft_rtreeformer"
        ...
        if sz == "large":
            ...
            if do_finetune and (not result_exists(run_dir, ft_key) or force):
                ...
                ft_model, ft_time = finetune_rtreeformer(
