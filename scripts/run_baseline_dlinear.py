"""
scripts/run_baseline_dlinear.py
====================================
Train All-Grid DLinear (("small", "base", "large")).
Also runs zero-shot and retrain adaptation experiments for the Large variant.

Usage
-----
    python scripts/run_baseline_dlinear.py
    python scripts/run_baseline_dlinear.py --sizes base
    python scripts/run_baseline_dlinear.py --no-adapt
    python scripts/run_baseline_dlinear.py --force
"""
...
from rtreeformer.model   import AllGridDLinear
...
def build_model(sz):
    s = MODEL_SIZES[sz]
    return AllGridDLinear(lookback=LOOKBACK, kernel_size=3, d_model=s["d_model"])
...
def run(sizes=SIZES, force=False, do_adapt=True):
    ...
    for sz in sizes:
        ...
        save_result(run_dir, key, preds, trues, {
            "model_name": f"All-Grid DLinear ({sz.capitalize()})",
        ...
        if sz == "large" and do_adapt:
            zs_key  = "adapt_zs_dlinear"
            ret_key = "adapt_retrain_dlinear"
            ...
            if not result_exists(run_dir, zs_key) or force:
                ...
                save_result(run_dir, zs_key, zs_p, zs_t,
                            {"model_name": "All-Grid DLinear (Large) Zero-Shot", "MAE": zm})
            ...
            if not result_exists(run_dir, ret_key) or force:
                ...
                _, rp, rt, rpe, rte, rt_s, _ = run_training(
                    "All-Grid DLinear (Large) Retrain",
                ...
                save_result(run_dir, ret_key, rpe, rte,
                            {"model_name": "All-Grid DLinear (Large) Retrain",
